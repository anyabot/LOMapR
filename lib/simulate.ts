// Round-1 team simulation: fixpoint application of battle/round-start passive
...
export type NoteKind =
  | 'enemy-target'   // targets the enemy side (no enemy team simulated)
  | 'event'          // fires on a mid-battle event, not at battle/round start
  | 'enemy-cond'     // condition reads enemy-side state
...
export interface SimTimelineGroup {
  ...
  tiles: number[];      // action order inside this round
}

export interface SimResult {
  units: SimUnitResult[];
  order: number[];      // tiles actionable in combat round 1, in action order
...
// ── static (round-1) trigger classification ──────────────────────────────────

// triggers resolvable before anyone acts in round 1
const STATIC_TRIGGERS = new Set([12, 13, 30, 27, 28, 29, 7, 5, 6, 25, 26, 21, 32, 33, 34]);
...
interface UnitState {
  ...
  hpFrac: number;       // configured current HP, 0..1
}

// grid depth is the column: 0 = back line, 1 = mid, 2 = front line
const depth = (tile: number) => tile % 3;
...
function evalCond(
  ...
  const allies = Array.from(units.values());
  const need = (n: number) => (Number.isFinite(n) ? n : NaN);

  switch (cond) {
    case 63: return true;
    // 66: NONE sentinel in global (no params); real "enemy has [buff]" in KR
    case 66: return c.vals.length === 0 ? true
      : { note: 'enemy-cond', detail: 'cond 66: enemy has buff' };
    ...
    case 2:  return depth(self.input.tile) === 2;   // front line
    case 3:  return depth(self.input.tile) === 1;   // mid line
    case 4:  return depth(self.input.tile) === 0;   // back line
    ...
    case 14: return c.vals.includes(tgt.input.unit.id);          // target is [char]
    case 15: return c.vals.some((v) => allies.some((u) => u.input.unit.id === v)); // [char] in battle
    case 16: return depth(tgt.input.tile) === 2;
    case 17: return depth(tgt.input.tile) === 1;
    case 18: return depth(tgt.input.tile) === 0;
    ...
    case 25: return { note: 'enemy-cond', detail: 'cond 25: enemy count' };
    case 26: return allies.length === need(threshold);           // allies = N
    case 27: return { note: 'enemy-cond', detail: 'cond 27: total unit count needs the enemy side' };
    case 28: return round >= need(threshold);
    case 29: return round <= need(threshold);
    case 30: return !c.vals.some((v) => allies.some((u) => u.input.unit.id === v)); // [char] NOT in battle
    case 32: return round === need(threshold);
    case 33: return allies.filter((u) => BODY_ORD[u.input.unit.body] === 0).length === need(threshold);
    case 34: return allies.filter((u) => BODY_ORD[u.input.unit.body] === 1).length === need(threshold);
    case 35: case 36: return { note: 'enemy-cond', detail: `cond ${cond}: enemy body count` };
    ...
    case 38: {                                                   // ≥ N allies are [class]
      ...
      return allies.filter((u) => ords.includes(CLASS_ORD[u.input.unit.type])).length >= need(threshold);
    }
    case 39: {                                                   // ≥ N allies are [role]
      const ords = c.vals.length ? c.vals.map(Number) : [];
      return allies.filter((u) => ords.includes(ROLE_ORD[u.input.unit.role])).length >= need(threshold);
    }
    case 40: return c.vals.map(Number).includes(CLASS_ORD[self.input.unit.type]); // self is [class]
    case 41: return round % 2 === 0;   // even round
    case 42: return round % 2 === 1;   // odd round
    case 43: case 58:                  // target missing any [buff]/[buff type]
      return countMatching(tgt, c.vals, c.names, c.condAttr) === 0;
    case 45: case 46: return { note: 'enemy-cond', detail: `cond ${cond}: enemy class/role count` };
    ...
    case 59: {                                                   // ≥ N ally nearby
      const adj = adjacentTiles(self.input.tile).filter((t2) => units.has(t2)).length;
      return adj >= (Number.isFinite(threshold) ? threshold : 1);
    }
    case 60: return adjacentTiles(self.input.tile).every((t2) => !units.has(t2)); // no ally nearby
    case 64: return { note: 'random', detail: 'cond 64: randomly picks one effect' };
    case 65: {                                                   // ≥ N of [char set] in battle
      const present = c.vals.filter((v) => allies.some((u) => u.input.unit.id === v)).length;
    ...
    case 68: return { note: 'enemy-cond', detail: 'cond 68: enemy buff count' };
    default:
...
function evalTrigger(b: SkillBuff, self: UnitState, units: Map<number, UnitState>, round: number): CondVerdict {
  switch (b.trigger) {
    case 12: case 13: case 30: return true;
    case 27: return depth(self.input.tile) === 2;   // front line
    case 28: return depth(self.input.tile) === 1;   // mid line
    case 29: return depth(self.input.tile) === 0;   // back line
    ...
    case 7: case 34: {   // {key} in allied squad / in grid (= sortied on the board)
      ...
      return Array.from(units.values()).some((u) => u.input.unit.id === key);
    }
...
interface Candidate {
  ...
  area: number[];
  center: number;
  appliedTo: Set<number>;    // tiles already granted this buff
  dead: boolean;             // permanently resolved (note emitted / fully applied)
}

const MAX_PASSES = 12;

export function simulateRound1(inputs: SimUnitInput[]): SimResult {
  const round = 1;
  const units = new Map<number, UnitState>();
  for (const inp of inputs) {
    units.set(inp.tile, {
      input: inp, applied: [], battle: { ...inp.stats.total },
  ...
  const noteOnce = (cand: Candidate, kind: NoteKind, detail: string) => {
    const u = units.get(cand.caster)!;
    notes.push({
      kind, tile: cand.caster, unitId: u.input.unit.id,
  ...
  for (const inp of inputs) {
    ...
    const push = (sourceName: string, sourceKind: 'skill' | 'equip',
      ...
      candidates.push({
        id: cid++, caster: inp.tile, sourceName, sourceKind, buff: b,
    ...
    for (const src of inp.passives) {
      for (const b of src.skill.buffs) {
        if (b.applyCond !== 64 && b.effectKey && claimed.has(b.effectKey)) {
          notes.push({
            kind: 'random', tile: inp.tile, unitId: inp.unit.id,
  ...
  for (pass = 1; pass <= MAX_PASSES; pass++) {
    ...
    for (const cand of candidates) {
      ...
      const self = units.get(cand.caster)!;
      const b = cand.buff;

      // enemy-side targets: note once, done (no enemy team is simulated)
      if (b.targetType === 3 || b.targetType === 4 || b.targetType === 9) {
        noteOnce(cand, 'enemy-target', `target ${b.targetType}: enemy side`);
        cand.dead = true;
        continue;
      }
      // random-pick marker
      ...
      // trigger
      if (!STATIC_TRIGGERS.has(b.trigger)) {
        const v = evalTrigger(b, self, units, round);
        if (v !== true) {
      ...
      let casterOk = true; let noted = false;
      for (const spec of specs) {
        if (TARGET_CONDS.has(spec.cond)) continue;
        const v = evalCond(spec, self, null, units, round);
        if (v === true) continue;
        casterOk = false;
        if (typeof v === 'object') { noteOnce(cand, v.note, v.detail); cand.dead = true; noted = true; }
        break;
      }
      if (!casterOk) { if (!noted) { /* retry next pass */ } continue; }

      // resolve targets
      let targets: number[] = [];
      switch (b.targetType) {
        case 0: targets = [cand.caster]; break;
        case 2: case 6:
          targets = mapAreaToTiles(cand.area, cand.center, cand.caster)
            .filter((t2) => units.has(t2));
          break;
        case 1: case 5: case 8:
          targets = Array.from(units.keys());
          break;
        default: targets = [cand.caster];
      }
      // body/class/role filters
      targets = targets.filter((t2) => {
        const u = units.get(t2)!.input.unit;
        if (b.filterBody?.length && !b.filterBody.includes(BODY_ORD[u.body])) return false;
        if (b.filterClass?.length && !b.filterClass.includes(CLASS_ORD[u.type])) return false;
        if (b.filterRole?.length && !b.filterRole.includes(ROLE_ORD[u.role])) return false;
        return true;
      });

      for (const t2 of targets) {
        if (cand.appliedTo.has(t2)) continue;
        const tu = units.get(t2)!;
        // per-target conditions
        let ok = true;
        for (const spec of specs) {
          if (!TARGET_CONDS.has(spec.cond)) continue;
          const v = evalCond(spec, self, tu, units, round);
          if (v === true) continue;
          ok = false;
          if (typeof v === 'object') { noteOnce(cand, v.note, v.detail); cand.dead = true; }
          break;
        ...
        if (b.overlapType === 3 && b.effectKey &&
          ...
          cand.appliedTo.add(t2);
          continue;
        }
        tu.applied.push({
          buff: b, sourceTile: cand.caster, sourceName: cand.sourceName,
          sourceKind: cand.sourceKind, pass, chance: b.rate < 1,
        });
        cand.appliedTo.add(t2);
        appliedThisPass++;
      }
  ...
  // ── final stats + AP / action timeline ──
  ...
  for (const [tile, u] of Array.from(units.entries())) {
    ...
    apMeta.set(tile, { battle, apAdd, roundAp, apSet, spd, cycles });
  }
  const firstCycle = Math.min(...Array.from(apMeta.values()).map((m) => m.cycles));
  const results: SimUnitResult[] = [];
  for (const [tile, u] of Array.from(units.entries())) {
    const { battle, apAdd, roundAp, apSet, spd, cycles } = apMeta.get(tile)!;
    ...
    results.push({
      tile: u.input.tile, unitId: u.input.unit.id, applied: u.applied,
      battle, delta, spd: battle.SPD, cycles, readyRound, ap, readyAp,
    });
  }
  results.sort((a, b) => a.tile - b.tile);

  const numpad = [7, 8, 9, 4, 5, 6, 1, 2, 3];
  const actionSort = (a: SimUnitResult, b: SimUnitResult) =>
    (b.readyAp - a.readyAp) || (b.spd - a.spd) || (numpad[b.tile] - numpad[a.tile]);
  ...
  const timeline = rounds.map((readyRound) => ({
    ...
    tiles: results.filter((r) => r.readyRound === readyRound).sort(actionSort).map((r) => r.tile),
  }));
  const order = timeline.find((g) => g.round === 1)?.tiles ?? [];

  return { units: results, order, timeline, firstCycle, notes, passes: pass };
}
